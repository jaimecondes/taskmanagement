...
export default function TaskList() {
  ...
  const [filter, setFilter] = useState<'all' | 'done' | 'notDone'>('all'); // ✅ filter state
  ...
  const [markTaskDone] = useMutation(MARK_TASK_DONE, { onCompleted: () => refetch() });
  const [markTasknotDone] = useMutation(MARK_TASK_NOT_DONE, { onCompleted: () => refetch() });
  const [deleteTask] = useMutation(DELETE_TASK, { onCompleted: () => refetch() });

  const handleMarkAsDone = async (id: string) => {
    try {
      const { data } = await markTaskDone({ variables: { id } });
      if (data?.markTaskDone) Alert.alert('Success', 'Task marked as done');
      else Alert.alert('Failed', 'Could not mark task as done.');
    } catch (err: any) {
  ...
  const handleMarkAsnotDone = async (id: string) => {
    try {
      ...
      if (data?.markTasknotDone) Alert.alert('Success', 'Task marked as not done');
      else Alert.alert('Failed', 'Could not mark task as not done.');
    } catch (err: any) {
      Alert.alert('Error', err.message || 'Something went wrong');
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (data?.deleteTask) Alert.alert('Success', data.deleteTask);
      else Alert.alert('Failed', 'Task not deleted.');
    } catch (err: any) {
      Alert.alert('Error', err.message || 'Something went wrong');
    }
  };
        ...
        renderItem={({ item }) => (
          <View style={styles.taskItem}>
              ...
              <Picker
                ...
                onValueChange={(value) => {
                  ...
                  if (item.done !== newStatus) {
                    const mutation = newStatus ? markTaskDone : markTasknotDone;
            ...
            <View style={styles.actions}>
              <TouchableOpacity style={[styles.button, styles.update]} onPress={() => handleUpdate(item)}>
